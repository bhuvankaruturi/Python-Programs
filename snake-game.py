...
def game():
    ...
    while key != 27:
		...
		#if the user clicks spacebar pause the game
        if key == ord(' '):                         
        ...
        if key != 27:
            snake.insert(0, [snake[0][0] + ((key == KEY_DOWN and 1) or (key == KEY_UP and -1)),
            snake[0][1] + ((key == KEY_LEFT and -1) or (key == KEY_RIGHT and 1))])
        ...
        if gameMode == 1:       #Arcade Mode
            #if the snake touches the boundary, GAME OVER!
            if snake[0][0] in [0, length-1] or snake[0][1] in [0, width-1]:
                print("GAME OVER!")
                break 
        else:       #Cruise Mode
            #if the snake goes to the boundaries re-enter it from other end
            if snake[0][0] == 0: snake[0][0] = length - 2
            if snake[0][0] == length - 1: snake[0][0] = 1
            if snake[0][1] == 0: snake[0][1] = width - 2
            if snake[0][1] == width - 1: snake[0][1] = 1
        
		#if the snake eats food, create food at some other random location
        if snake[0] == food: 
            score += 1
            food = []
            while food == []:
            ...
            win.addch(food[0], food[1], '*')
        else:	#if the food is not eaten, pop the last block off the snake
            last = snake.pop() 
            win.addch(last[0], last[1], ' ')
        win.addch(snake[0][0], snake[0][1], '#')
		
	#If game exited by user, display 'Exited game!' message
    if key == 27: print('Exited game!')
	
    print('Score:', score)
	
	#reset command prompt window attributes to default values
    curses.echo()
    curses.nocbreak()
    curses.endwin()
...
def selectGameMode():
    print('enter 1 for arcade')
    print('enter 2 for cruise')
    gameMode = input('select mode: ')
    if (int(gameMode) not in [1, 2]):
